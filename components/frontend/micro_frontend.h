 ...
 *
 * 将 PCM 音频流转换为 Mel-scale 40维 spectrogram（uint8），
 * 参数与 micro-wake-word audio_utils.py 严格一致。
 *
 * 流水线:
 *   PCM → Frame(30ms/10ms) → Hann Window → 512-FFT → Mel Filterbank(40ch)
 *       → Log → PCAN → uint8
 *
 * 参考:
...
#define MF_NUM_CHANNELS     40      /* Mel 滤波器数量 */
...
/**
 * @brief 处理一帧 PCM 数据，输出 40 维 uint8 spectrogram
 *
 ...
 * @param features  输出 uint8[40] Mel spectrogram
 * @return ESP_OK 成功
 */
