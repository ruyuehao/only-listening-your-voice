 ...
 *
 * 累积 micro_frontend 输出的逐帧 13 维 spectrogram，
 * 为 KWS (100 帧) 和 SV (40 帧) 提供不同长度的滑动窗口视图。
 *
 * 由于 KWS 和 SV 使用同一段音频，从同一缓冲区取不同帧数即可：
 *   KWS: 最近 100 帧 → [100 × 13] uint8 spectrogram
 *   SV:  最近 40 帧  → [40 × 13]  uint8 spectrogram
 */
...
#define FB_NUM_CHANNELS     13      /* 与 micro_frontend MF_NUM_CHANNELS 一致 */
 ...
 *
 * @param frame  uint8[13] Mel spectrogram (1 帧 @ 10ms)
 * @return ESP_OK 成功
 */
 ...
 *
 * @param[out] dst     目标缓冲区 [N × 13] uint8
